...
frontend/                  — Svelte 5 SPA (Vite + TypeScript + pnpm)
  ...
  src/lib/
    ...
    MergePanel.svelte      — 3-pane conflict editor (ours | result | theirs). 3 raw CM6 EditorViews — no @codemirror/merge dep. blockTracker StateField maps per-block positions through edits (mapPos assoc 1/-1, degenerate-range clamp for select-all-and-type). takeBlock arrow-click: idempotent same-side, leading/trailing \n separator math at doc boundaries, srcEmpty (not !insert — blank lines are valid content). applyBlock/editInside/restoreBlock StateEffects + invertedEffects undo inverse (Cmd+Z restores text AND source tag). Center seeded with theirs.
...
- **Use `\x1F` (unit separator)** as the field delimiter in jj templates, not tabs. Tabs can appear in commit descriptions and break parsing. For sub-fields within a field (e.g. `name`/`remote` inside each remote-bookmark entry), use **`\x1E` (record separator)** — Git refs can't contain control chars per `git-check-ref-format`, unlike `@` which CAN appear in git-created branch names. Split at parse time (Go parser) — `\x1E` never crosses the wire.
