...
- **Keyboard/mouse parity: `handleInlineNav` is the per-mode source of truth.** Its three-way (`split` → no nav, `squash` → `selectRevisionCursorOnly` diff-frozen-on-source, `rebase` → `selectRevision` diff-follows-destination) defines mode semantics. Any mouse-path fix (`onselect` prop, context-menu) must mirror this discrimination — `inlineMode` is WRONG because it lumps rebase with squash/split. When fixing a mouse-path bug, grep for where keyboard handles the same action and match its per-mode behavior. Grep the guard expression (`activeView = 'branches'`), not the fix site — `switchToBranchesView()` centralizes branches-entry semantics the way `switchToLogView()` does for log.
- **Effect declaration order + untracked reads = permanent skip.** Effects fire in declaration order within a batch. If effect-A (declared first) does `untrack(() => stateX)` for a dedup-check and effect-B (declared later) clears `stateX` as part of reset, then on a trigger that invalidates both: A reads the stale pre-reset `stateX`, dedup says "already have it", A skips work; B clears; A never re-fires (untracked). See `conflictFileDiffs` in DiffPanel — the fix is to track WHICH identity the cached entries are for and clear synchronously in effect-A when identity differs. The existing comment documenting one manifestation of an effect-ordering hazard does NOT mean the other manifestations are covered.
