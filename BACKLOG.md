...
- [x] **Subtle reload over SSH** — mutations over SSH cause full-screen "Loading…" state because each jj call takes ~440ms and the `loading` flag flips. Fixed in `RevisionGraph.svelte:49-52` (`isRefreshing` derived) + `:270-278` (refresh-bar + stale-content preservation). `loader.loading` is now a "refreshing" indicator, not a content gate; initial-load shows spinner, refresh dims + top progress bar.
