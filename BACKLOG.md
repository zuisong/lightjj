...
- [ ] **Sync `compute` variant for `createDiffDerivation`** — `highlightFile` now has a sync body but stays `async` only because the factory's `compute: (file, isStale) => Promise<T>` contract. A `computeSync?: (file) => T` option would let `run()` skip microtask scheduling entirely for sync computations. Only highlights would use it; word-diff LCS stays async.
