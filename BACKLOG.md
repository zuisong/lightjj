...
- [ ] **Cross-repo config bleed** (Medium) — `remoteVisibility` is stored in global `config.json` but is semantically per-repo — toggling origin visibility in tab A bleeds into tab B's unrelated repo. Proper fix needs per-tab config or repo-keyed sub-objects.
- [ ] **Audit localStorage usage for port-randomization breakage** (Small) — localStorage is scoped to origin (`localhost:PORT`). lightjj randomizes ports on restart → all localStorage state lost each launch. Audit `recent-actions.svelte.ts`, `config.svelte.ts` write-through cache. Either migrate to server-side config or accept the ephemeral semantics and document.
