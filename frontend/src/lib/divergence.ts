...
// --- Refined classification (post-tree-delta) ---
// Originally extracted from DivergencePanel.svelte's $derived.by blocks so the
// decision tree is unit-testable; the panel's reactive chain shrinks to
// one-liners that call these.
 ...
 *
 *  The fileUnion-subtraction step (refineRebaseKind above) is what prevents
 *  trunk churn from being miscounted as edit-drift: a diff-parent group with
 *  ONLY trunk-touched files in its cross-diff is still pure-rebase. See
